...
def edit_event(request: HttpRequest):
    ...
    try:
        if not event_id:
            return HttpResponseBadRequest(content='<h1>Bad Request</h1><h3>Param "id" is missing.</h3>')
        event_id = int(event_id)
    except ValueError:
        return HttpResponseBadRequest(content='<h1>Bad Request</h1><h3>Invalid param value.</h3>')
